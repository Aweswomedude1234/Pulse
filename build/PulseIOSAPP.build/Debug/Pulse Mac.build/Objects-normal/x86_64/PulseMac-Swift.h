...
#ifndef PULSEMAC_SWIFT_H
#define PULSEMAC_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="PulseMac",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class NSApplication;
SWIFT_CLASS("_TtC8PulseMac14MacAppDelegate")
...
/// Closing the window should not quit: Pulse keeps listening from the menu
/// bar. Quitting is explicit, via ⌘Q or the menu bar extra.
...
SWIFT_CLASS("_TtC8PulseMac19NotificationManager")
@interface NotificationManager : NSObject <UNUserNotificationCenterDelegate>
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
/// Show notifications even when app is foregrounded — important for this app
- (void)userNotificationCenter:(UNUserNotificationCenter * _Nonnull)center willPresentNotification:(UNNotification * _Nonnull)notification withCompletionHandler:(void (^ _Nonnull)(UNNotificationPresentationOptions))completionHandler;
- (void)userNotificationCenter:(UNUserNotificationCenter * _Nonnull)center didReceiveNotificationResponse:(UNNotificationResponse * _Nonnull)response withCompletionHandler:(void (^ _Nonnull)(void))completionHandler;
@end

/// macOS cannot pair with an Apple Watch (WatchConnectivity is iOS-only), so the
/// Mac build gets a no-op with the same surface. Keeping the type rather than
/// conditionalising every call site means <code>PulseSessionCoordinator</code> and the
/// views stay identical across platforms.
SWIFT_CLASS("_TtC8PulseMac24WatchConnectivityManager")
